...
const FInterpCurvePointVector USplineComponent::DummyPointUpVector(0.0f, FVector::UpVector);
...
#endif
{
	...
	SplineUpVInfo_DEPRECATED = SplineCurves.UpVector;
...
/*
void USplineComponent::CheckAndGenUpVectorSplineFromRotation()
{	
	for (int i = 0; i < SplineCurves.Rotation.Points.Num(); i++)
	{
		if (!SplineCurves.UpVector.Points.IsValidIndex(i))
		SplineCurves.UpVector.Points.Emplace(
...
void USplineComponent::SetDefaultSpline()
{
	...
	SplineCurves.UpVector.Points.Reset(10);
	...
	SplineCurves.UpVector.Points.Emplace(0.0f, FVector(0, 0, 1), FVector::ZeroVector, FVector::ZeroVector, CIM_CurveAuto);
...
void USplineComponent::Serialize(FArchive& Ar)
{
		...
		Ar.IsLoading())
	{
		...
		SplineCurves.UpVector = SplineUpVInfo_DEPRECATED;
	...
	if (ArchiveUEVersion < VER_UE4_INTERPCURVE_SUPPORTS_LOOPING)
	{
		...
		// Fill the other splines with some defaults
		SplineCurves.Rotation.Points.Reset(NumPoints);
		SplineCurves.UpVector.Points.Reset(NumPoints);
		SplineCurves.Scale.Points.Reset(NumPoints);
		for (int32 Count = 0; Count < NumPoints; Count++)
		{
			SplineCurves.Rotation.Points.Emplace(0.0f, FQuat::Identity, FQuat::Identity, FQuat::Identity, CIM_CurveAuto);
			SplineCurves.UpVector.Points.Emplace(0.0f, FVector::UpVector, FVector::ZeroVector, FVector::ZeroVector, CIM_CurveAuto);
			SplineCurves.Scale.Points.Emplace(0.0f, FVector(1.0f), FVector::ZeroVector, FVector::ZeroVector, CIM_CurveAuto);
		}
	...
	// For instances lacking an UpVector spline
	if (SplineCurves.UpVector.Points.Num() != SplineCurves.Rotation.Points.Num())
	{
		SplineCurves.UpVector.Points.SetNum(SplineCurves.Rotation.Points.Num());
		for (int i = 0; i < SplineCurves.Rotation.Points.Num(); i++)
		{
			SplineCurves.UpVector.Points[i] = FInterpCurvePoint(
				SplineCurves.Rotation.Points[i].InVal,
				SplineCurves.Rotation.Points[i].OutVal.RotateVector(FVector::UpVector),
				FVector::ZeroVector,
				FVector::ZeroVector,
...
void FSplineCurves::UpdateSpline(bool bClosedLoop, bool bStationaryEndpoints, int32 ReparamStepsPerSegment, bool bLoopPositionOverride, float LoopPosition, const FVector& Scale3D)
{
	...
	check(Rotation.Points.Num() == NumPoints && UpVector.Points.Num() == NumPoints && Scale.Points.Num() == NumPoints);
	...
	UpVector.AutoSetTangents(0.0f, bStationaryEndpoints);
...
FQuat USplineComponent::GetQuaternionAtSplineInputKey(float InKey, ESplineCoordinateSpace::Type CoordinateSpace) const
{
	...
	const FVector UpVector = SplineCurves.UpVector.Eval(InKey, FVector::UpVector).GetSafeNormal();//Quat.RotateVector(DefaultUpVector);
...
void USplineComponent::ClearSplinePoints(bool bUpdateSpline)
{
	...
	SplineCurves.UpVector.Points.Reset();
...
void USplineComponent::AddPoint(const FSplinePoint& InSplinePoint, bool bUpdateSpline)
{
	...
	SplineCurves.UpVector.Points.Insert(FInterpCurvePoint<FVector>(
		InSplinePoint.InputKey,
		InSplinePoint.Rotation.RotateVector(FVector::UpVector),//UKismetMathLibrary::GetUpVector(InSplinePoint.Rotation),
		InSplinePoint.Rotation.RotateVector(FVector::ForwardVector),//InSplinePoint.Rotation.UpVector(),
		InSplinePoint.Rotation.RotateVector(FVector::ForwardVector),//InSplinePoint.Rotation.UpVector(),
		CIM_CurveAuto
		),
		Index);
...
void USplineComponent::AddPoints(const TArray<FSplinePoint>& InSplinePoints, bool bUpdateSpline)
{
	...
	// Position, UpVector, Rotation, and Scale will all grow together.
	SplineCurves.Position.Points.Reserve(NumPoints);
	SplineCurves.UpVector.Points.Reserve(NumPoints);
...
void USplineComponent::AddSplinePoint(const FVector& Position, ESplineCoordinateSpace::Type CoordinateSpace, bool bUpdateSpline)
{
	...
	SplineCurves.UpVector.Points.Emplace(InKey, FVector(0, 0, 1), FVector::ZeroVector, FVector::ZeroVector, CIM_CurveAuto);
...
void USplineComponent::AddSplinePointAtIndex(const FVector& Position, int32 Index, ESplineCoordinateSpace::Type CoordinateSpace, bool bUpdateSpline)
{
	...
	if (Index >= 0 && Index <= NumPoints)
	{
		...
		SplineCurves.UpVector.Points.Insert(FInterpCurvePoint<FVector>(InKey, FVector(0, 0, 1), FVector::ZeroVector, FVector::ZeroVector, CIM_CurveAuto), Index);
		...
		for (int I = Index + 1; I < NumPoints; ++I)
		{
			...
			SplineCurves.UpVector.Points[I].InVal += 1.0f;
...
void USplineComponent::RemoveSplinePoint(int32 Index, bool bUpdateSpline)
{
	...
	if (Index >= 0 && Index < NumPoints)
	{
		...
		SplineCurves.UpVector.Points.RemoveAt(Index, 1, EAllowShrinking::No);
		...
		while (Index < NumPoints)
		{
			...
			SplineCurves.UpVector.Points[Index].InVal -= 1.0f;
...
void USplineComponent::SetSplinePoints(const TArray<FVector>& Points, ESplineCoordinateSpace::Type CoordinateSpace, bool bUpdateSpline)
{
	...
	for (const auto& Point : Points)
	{
		...
		SplineCurves.UpVector.Points.Emplace(InputKey, FVector(0, 0, 1), FVector::ZeroVector, FVector::ZeroVector, CIM_CurveAuto);
...
void USplineComponent::SetUpVectorAtSplinePoint(int32 PointIndex, const FVector& InUpVector, ESplineCoordinateSpace::Type CoordinateSpace, bool bUpdateSpline)
{
	...
	if ((PointIndex >= 0) && (PointIndex < NumPoints))
	{
		...
		SplineCurves.UpVector.Points[PointIndex].OutVal = TransformedUpVector;
		UE_LOG(LogTemp, Display, TEXT("SetUpVectorAtSplinePoint"));
...
void USplineComponent::SetRotationAtSplinePoint(int32 PointIndex, const FRotator& InRotation, ESplineCoordinateSpace::Type CoordinateSpace, bool bUpdateSpline /*= true*/)
{
	if (SplineCurves.Rotation.Points.IsValidIndex(PointIndex))
	{
		...
		UE_LOG(LogTemp, Display, TEXT("SetRotationAtSplinePoint"));
